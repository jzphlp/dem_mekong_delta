...
#Here's a Python equivalent of the remove_off_terrain_objects function from the Rust source code you provided:
import numpy as np
from whitebox_tools import WhiteboxTools

# instantiate a WhiteboxTools object
wbt = WhiteboxTools()

def remove_off_terrain_objects(input_file, output_file, filter_size=9, slope_threshold=15.0):
    # read the input raster file
    wbt.load_data(input_file)
    # get the no data value
    no_data = wbt.get_nodata_value(input_file)
    # get the data type
    data_type = wbt.get_data_type(input_file)

    # create the filter matrix
    filt = np.ones((filter_size, filter_size), dtype=np.float32)

    # calculate the slope and aspect grids
    slope_file = wbt.slope(input_file)
    aspect_file = wbt.aspect(input_file)

    # read the slope grid
    wbt.load_data(slope_file)
    slope = wbt.get_data()
    slope[np.isnan(slope)] = no_data

    # read the aspect grid
    wbt.load_data(aspect_file)
    aspect = wbt.get_data()
    aspect[np.isnan(aspect)] = no_data

    # calculate the x and y gradients
    x_kernel = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
    y_kernel = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]])

    x_slope = wbt.filter(x_kernel, slope, filter_size, filter_size, no_data, data_type)
    y_slope = wbt.filter(y_kernel, slope, filter_size, filter_size, no_data, data_type)

    # calculate the slope aspect ratio
    sar = np.arctan(np.sqrt(x_slope**2 + y_slope**2) / slope) * (180.0 / np.pi)

    # create the output grid
    wbt.create_new_raster(output_file, wbt.num_rows, wbt.num_cols, wbt.num_bands, wbt.data_type, no_data)

    # loop through each grid cell
    for row in range(wbt.num_rows):
        for col in range(wbt.num_cols):
            if slope[row, col] != no_data:
                if sar[row, col] < slope_threshold:
                    wbt.assign_value(output_file, row, col, no_data)
                else:
                    wbt.assign_value(output_file, row, col, wbt.get_value(input_file, row, col))
            else:
                wbt.assign_value(output_file, row, col, no_data)
